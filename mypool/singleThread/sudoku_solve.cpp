...
class ThreadPool{	
	...
	private:
		...
		std::string resultSet[64001];// 结果集 
					...
					{				
						while(flag)
						{
							...
							if(d.solve())
							{
								...
								string res = Result().getString(board);// 结果转换 
								result[index] = res;// 将结果放入结果集
							}														
						}
					}
				);
...
int main(){
	 ...
	 ThreadPool threadPool(2);//--------------------------------------------------实例化一个具有3个核心线程的线程池 
	 ...
	 { 
		 ...
		 { 
		    sleep(0.001);//-------------------------------------------------------等待直到结果集有该次结果		 	
		 }
		std::cout<<result[i]<<endl;//-------------------------------将求解结果输出到STD::COUT 
	 } 	 
