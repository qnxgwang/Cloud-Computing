#include<iostream>
#include<fstream>
#include<memory.h>
#include<assert.h>
#include<thread>
#include <sys/time.h>
#include<queue> 
#include<map>
#include<mutex>
#include<unistd.h> 
#include<condition_variable>
#define N 81
#define DEFAULT_SOLVE_NUM 1000
#define FILE_NAME "test1000" 
using namespace std;
/**
 * @author = qnxg_wang 
 * @version = 1.0
 * @link = https://github.com/qnxgwang
 * @see README.md
 * taskset -c 1 ./sudoku_solve&
 * ./sudoku_solve
 **/
const int kMaxNodes = 1 + 81*4 + 9*9*9*4;
const int kMaxColumns = 400;
const int kRow = 100, kCol = 200, kBox = 300;

struct Node;
typedef Node Column;

string file_name;
int solve_num;

struct Node
{
    Node* left;
    Node* right;
    Node* up;
    Node* down;
    Column* col;
    int name;
    int size;
};

struct Dance
{
    Column* root_;
    int*    inout_;
    Column* columns_[400];
    vector<Node*> stack_;
    Node    nodes_[kMaxNodes];
    int     cur_node_;

    Column* new_column(int n = 0)
    {
        assert(cur_node_ < kMaxNodes);
        Column* c = &nodes_[cur_node_++];
        memset(c, 0, sizeof(Column));
        c->left = c;
        c->right = c;
        c->up = c;
        c->down = c;
        c->col = c;
        c->name = n;
        return c;
    }

    void append_column(int n)
    {
        assert(columns_[n] == NULL);

        Column* c = new_column(n);
        put_left(root_, c);
        columns_[n] = c;
    }

    Node* new_row(int col)
    {
        assert(columns_[col] != NULL);
        assert(cur_node_ < kMaxNodes);

        Node* r = &nodes_[cur_node_++];

        //Node* r = new Node;
        memset(r, 0, sizeof(Node));
        r->left = r;
        r->right = r;
        r->up = r;
        r->down = r;
        r->name = col;
        r->col = columns_[col];
        put_up(r->col, r);
        return r;
    }

    int get_row_col(int row, int val)
    {
        return kRow+row*10+val;
    }

    int get_col_col(int col, int val)
    {
        return kCol+col*10+val;
    }

    int get_box_col(int box, int val)
    {
        return kBox+box*10+val;
    }

    Dance(int inout[81]) : inout_(inout), cur_node_(0)
    {
        stack_.reserve(100);

        root_ = new_column();
        root_->left = root_->right = root_;
        memset(columns_, 0, sizeof(columns_));

        bool rows[N][10] = {false};
        bool cols[N][10] = {false};
        bool boxes[N][10] = {false};

        for (int i = 0; i < N; ++i) {
            int row = i / 9;
            int col = i % 9;
            int box = row/3*3 + col/3;
            int val = inout[i];
            rows[row][val] = true;
            cols[col][val] = true;
            boxes[box][val] = true;
        }

        for (int i = 0; i < N; ++i) {
            if (inout[i] == 0) {
                append_column(i);
            }
        }

        for (int i = 0; i < 9; ++i) {
            for (int v = 1; v < 10; ++v) {
                if (!rows[i][v])
                    append_column(get_row_col(i, v));
                if (!cols[i][v])
                    append_column(get_col_col(i, v));
                if (!boxes[i][v])
                    append_column(get_box_col(i, v));
            }
        }

        for (int i = 0; i < N; ++i) {
            if (inout[i] == 0) {
                int row = i / 9;
                int col = i % 9;
                int box = row/3*3 + col/3;
                //int val = inout[i];
                for (int v = 1; v < 10; ++v) {
                    if (!(rows[row][v] || cols[col][v] || boxes[box][v])) {
                        Node* n0 = new_row(i);
                        Node* nr = new_row(get_row_col(row, v));
                        Node* nc = new_row(get_col_col(col, v));
                        Node* nb = new_row(get_box_col(box, v));
                        put_left(n0, nr);
                        put_left(n0, nc);
                        put_left(n0, nb);
                    }
                }
            }
        }
    }

    Column* get_min_column()
    {
        Column* c = root_->right;
        int min_size = c->size;
        if (min_size > 1) {
            for (Column* cc = c->right; cc != root_; cc = cc->right) {
                if (min_size > cc->size) {
                    c = cc;
                    min_size = cc->size;
                    if (min_size <= 1)
                        break;
                }
            }
        }
        return c;
    }

    void cover(Column* c)
    {
        c->right->left = c->left;
        c->left->right = c->right;
        for (Node* row = c->down; row != c; row = row->down) {
            for (Node* j = row->right; j != row; j = j->right) {
                j->down->up = j->up;
                j->up->down = j->down;
                j->col->size--;
            }
        }
    }

    void uncover(Column* c)
    {
        for (Node* row = c->up; row != c; row = row->up) {
            for (Node* j = row->left; j != row; j = j->left) {
                j->col->size++;
                j->down->up = j;
                j->up->down = j;
            }
        }
        c->right->left = c;
        c->left->right = c;
    }

    bool solve()
    {
        if (root_->left == root_) {
            for (size_t i = 0; i < stack_.size(); ++i) {
                Node* n = stack_[i];
                int cell = -1;
                int val = -1;
                while (cell == -1 || val == -1) {
                    if (n->name < 100)
                        cell = n->name;
                    else
                        val = n->name % 10;
                    n = n->right;
                }

                //assert(cell != -1 && val != -1);
                inout_[cell] = val;
            }
            return true;
        }

        Column* const col = get_min_column();
        cover(col);
        for (Node* row = col->down; row != col; row = row->down) {
            stack_.push_back(row);
            for (Node* j = row->right; j != row; j = j->right) {
                cover(j->col);
            }
            if (solve()) {
                return true;
            }
            stack_.pop_back();
            for (Node* j = row->left; j != row; j = j->left) {
                uncover(j->col);
            }
        }
        uncover(col);
        return false;
    }

    void put_left(Column* old, Column* nnew)
    {
        nnew->left = old->left;
        nnew->right = old;
        old->left->right = nnew;
        old->left = nnew;
    }

    void put_up(Column* old, Node* nnew)
    {
        nnew->up = old->up;
        nnew->down = old;
        old->up->down = nnew;
        old->up = nnew;
        old->size++;
        nnew->col = old;
    }
};
class Result {
public:
	/**
	 * @brief 结果接口 
	 * 
	 * @param board 
	 * 
	 * @return 
	 **/
    static string getString(int* board) {
        string s = "";
        for(int i=0;i<81;i++){
			s+=(board[i]+'0');
		}
        return s;
    }
};
class Task{
	
	private:		
		int board[81]; //数独矩阵 
		
		int index; //任务索引 
		
	public:	
	    /**
		 * @brief 设置数独矩阵的值 
		 * 
		 * @param i 数独矩阵的下标 
		 * @param value 对应下标的值 
		 **/
		void setBoard(int i,int value)
		{
			board[i] = value;
		}
		
		int getBoard(int i)
		{
			return board[i];
		}
		/**
		 * @brief 任务索引 
		 * 
		 * @param i 
		 **/
		void setIndex(int i)
		{
			index = i;
		}
		
		int getIndex()
		{
			return index;
		}
	
};
class ThreadPool{	

	private:
		int coreNum;
		
		bool flag = true;
		
		std::queue<Task> taskList;// 任务队列 
		
		std::string resultSet[1001];// 结果集 
		
		std::vector<thread> threadList;// 线程队列 
		
		std::mutex mtx; // 互斥量,保护产品缓冲区
		
	    std::condition_variable repo_not_full; // 条件变量, 指示产品缓冲区不为满.
	    
	    std::condition_variable repo_not_empty; // 条件变量, 指示产品缓冲区不为空.
	    
	public:   
	    /**
		 * @brief 线程池构造函数 
		 * 
		 * @param corenum 核心线程数量 
		 * 
		 * @return 
		 **/
		ThreadPool(int corenum)
		{			
		    coreNum = corenum;
			ProducerThreadStart();
			ConsumerThreadStart(corenum);			
		}	
        /**
		 * @brief 是否存在数独求解结果 
		 * 
		 * @param index 索引 
		 * 
		 * @return bool
		 **/
		bool hasResult(int index)
		{
			return (resultSet[index].length() != 0);
		}
		/**
		 * @brief 得到数独求解结果 
		 * 
		 * @param index 索引 
		 * 
		 * @return string 
		 **/
		string getResult(int index)
		{
			return resultSet[index];		 	
		}
		/**
		 * @brief 返回生产者线程的一个引用 
		 * 
		 * @return thread&
		 **/
		thread& getProducerThread()
		{
			return threadList.front();
		}
		/**
		 * @brief 销毁线程池 
		 * 
		 **/
		void shutDownNow()
		{
			flag = false;
			for(int i=0;i<=coreNum;i++){
				thread& t = threadList.front();				
				threadList.erase(threadList.begin());
				delete &t;
			}		
		}
	protected:	
	    /**
		 * @brief 生产者线程 
		 * 
		 **/
		void ProducerThreadStart(){
			threadList.emplace_back(
			    [this]
			    {
					string file = file_name;	
					fstream fin;	
					fin.open(file.c_str(), ios::in);
					Task task; 
					solve_num = 0;
					string data;
					while(getline(fin,data))
					{
						solve_num++;
						for(int i=0;i<81;i++) //设置数独矩阵的初始值 
						{
							task.setBoard(i,data[i]-'0');
						}
						task.setIndex(solve_num); //设置任务的索引 
						std::unique_lock<std::mutex> lock(mtx); //获取锁 							
						while(taskList.size() == 10)
						{
							repo_not_full.wait(lock); //等待 
						}
						//std::cout<<"-------------生产者提交任务:-------------"<<solveNUM<<endl;
						taskList.push(task); //提交任务 
						repo_not_empty.notify_all(); //通知消费者 
						lock.unlock(); // 释放锁 						
					}					
				}			    			
			);		
		}	
		/**
		 * @brief 消费者线程 
		 * 
		 * @param corenum 核心线程数量 
		 **/
		void ConsumerThreadStart(int corenum){
			for(int i=1; i<=corenum; i++)
			{
				threadList.emplace_back(
					[this]
					{				
						while(flag)
						{
							std::unique_lock<std::mutex> lock(mtx);// 获取锁 
							while(taskList.empty())
							{
								repo_not_empty.wait(lock);// 等待 
							}
							Task task = taskList.front();// 取任务 							
							taskList.pop();// 销毁任务 
							repo_not_full.notify_all();// 通知生产者 
							lock.unlock();// 释放锁 
							int index = task.getIndex();// 任务索引 							
							int board[81];
							for(int i=0;i<81;i++)
							{
								board[i] = task.getBoard(i);// 取数独矩阵 
							}
							Dance d(board);// 舞蹈链法 
							if(d.solve())
							{
								//std::cout<<"-------------消费者求解数独:-------------"<<index<<endl;
								string result = Result().getString(board);// 结果转换 
								resultSet[index] = result;// 将结果放入结果集
							}														
						}
					}
				);
			}
		}				
		
};
int64_t now()
{
  struct timeval tv;
  gettimeofday(&tv, NULL);
  return tv.tv_sec * 1000000 + tv.tv_usec;
}
int main(){
	 //getline(cin,file_name);//----------------------------------------------------读取文件名字 
	 file_name = "test1000";
	 int m;
	 cin>>m;
	 int64_t start = now();//-----------------------------------------------------开始计时 	 
	 ThreadPool threadPool(m);//--------------------------------------------------实例化一个具有3个核心线程的线程池 
	 thread& t = threadPool.getProducerThread();
	 t.join();//------------------------------------------------------------------等待直到生产者线程结束 
	 for(int i=1; i<=solve_num; i++)
	 { 
	 	while(!threadPool.hasResult(i))
		 { 
		    sleep(0.00001);//-------------------------------------------------------等待直到结果集有该次结果		 	
		 }
		std::cout<<threadPool.getResult(i)<<endl;//-------------------------------将求解结果输出到STD::COUT 
	 }	  
	 int64_t end = now();//-------------------------------------------------------结束计时
	 std::cout<<(end-start)/1000000.0<<endl;//--------------------------------------打印计时
	 threadPool.shutDownNow();//----------------------------------------------------销毁线程池
	 return 0; 
}
